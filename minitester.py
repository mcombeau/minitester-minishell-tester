...
test_dir = 'test_cwd'
bash_dir = 'test_cwd/bash'
mini_dir = 'test_cwd/minishell'
...
def output_files_ok():
    ...
    for root, dirs, files in os.walk(bash_dir):
        for file in files:
            bash_files_set.add(os.path.relpath(os.path.join(root, file), bash_dir))
    
    for root, dirs, files in os.walk(mini_dir):
        for file in files:
            mini_files_set.add(os.path.relpath(os.path.join(root, file), mini_dir))
    
    bash_files_set = {file for file in bash_files_set if os.path.basename(file) not in test_files}
    mini_files_set = {file for file in mini_files_set if os.path.basename(file) not in test_files}
    
    if bash_files_set != mini_files_set:
        print('Bash and minishell have different output files!')
        print(f'bash files: {bash_files_set}')
        print(f'mini files: {mini_files_set}')
        all_files_ok = False

    for file in bash_files_set:
...
def setup_test_files(path):
    existing_dir = os.path.join(path, 'existing_dir')
    os.makedirs(existing_dir, exist_ok=True)

    forbidden_dir = os.path.join(path, 'forbidden_dir')
    os.makedirs(forbidden_dir, exist_ok=True)
    os.chmod(forbidden_dir, 0o000)

    existing_file = os.path.join(path, 'existing_file')
    with open(existing_file, 'w') as file:
        file.write('This is a file that exists with r/w permissions')

    forbidden_file = os.path.join(path, 'forbidden_file')
    with open(forbidden_file, 'w') as file:
        file.write('This is a forbidden file')
    os.chmod(forbidden_file, 0o000)

    valid_infile_1 = os.path.join(path, 'valid_infile_1')
    with open(valid_infile_1, 'w') as file:
        file.write('Take this kiss upon the brow!\nAnd, in parting from you now,\nThus much let me avow-\nYou are not wrong, who deem\nThat my days have been a dream;\nYet if hope has flown away\nIn a night, or in a day,\nIn a vision, or in none,\nIs it therefore the less gone?\nAll that we see or seem\nIs but a dream within a dream.\n\nI stand amid the roar\nOf a surf-tormented shore,\nAnd I hold within my hand\nGrains of the golden sand-\nHow few! yet how they creep\nThrough my fingers to the deep,\nWhile I weep- while I weep!\nO God! can I not grasp\nThem with a tighter clasp?\nO God! can I not save\nOne from the pitiless wave?\nIs all that we see or seem\nBut a dream within a dream?\n\nEdgar Allan Poe\nA Dream Within a Dream')

    valid_infile_2 = os.path.join(path, 'valid_infile_2')
    with open(valid_infile_2, 'w') as file:
        head_output = subprocess.run(['head', '-n', '12'], input=subprocess.run(['man', 'bash'], stdout=subprocess.PIPE, text=True).stdout, text=True, stdout=subprocess.PIPE).stdout
        file.write(head_output)

    executable_file = os.path.join(path, 'executable_file')
    with open(executable_file, 'w') as file:
